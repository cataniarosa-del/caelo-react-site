...
const verticals = {
  passweb: {
    ...
    title: "CAELO PassWeb",
    status: "Completo, validato e pronto alla scalabilità",
  ...
  return (
    <Layout>
      <section className="py-20 md:py-28 bg-background">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={springPresets.gentle} className="max-w-5xl mx-auto">
            ...
            <h1 className="text-4xl md:text-6xl font-bold tracking-tight mb-6">{vertical.title}</h1>
            <div className="inline-flex rounded-2xl border border-primary/20 bg-primary/5 px-5 py-3 font-semibold mb-8">{vertical.status}</div>
      ...
      <section className="py-20 bg-muted/30">
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto grid lg:grid-cols-[1.1fr_0.9fr] gap-10 items-start">
            <div><h2 className="text-3xl md:text-4xl font-bold mb-6">Che cosa governa</h2><p className="text-lg text-muted-foreground leading-relaxed">{vertical.description}</p></div>
            <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }} className="rounded-3xl border border-border bg-card p-8 shadow-sm">
              <h2 className="text-2xl font-bold mb-6">Capacità principali</h2>
      ...
      <section className="py-20 bg-background">
        <div className="container mx-auto px-4"><div className="max-w-5xl mx-auto rounded-[2rem] border border-border bg-card p-8 md:p-12"><h2 className="text-3xl md:text-4xl font-bold mb-6">Valore del verticale</h2><p className="text-xl text-muted-foreground leading-relaxed">{vertical.value}</p></div></div>
      </section>
    </Layout>
  );
}
